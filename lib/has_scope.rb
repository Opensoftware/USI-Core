module HasScope
  ...
  module ClassMethods
    ...
    #   end
    #
    def has_scope(klazz, *scopes, &block)
      ...
      self.scopes_configuration[klazz.name] ||= {}

      scopes.each do |scope|
        self.scopes_configuration[klazz.name][scope] ||= { :as => scope, :type => :default, :block => block }.merge(options)
        #        self.scopes_configuration[scope] = self.scopes_configuration[scope]
      end
    end
  end
  ...
  def apply_scopes(target, hash=params)
    return target unless scopes_configuration.try(:[], target.name)

    self.scopes_configuration[target.name].each do |scope, options|
      ...
      #      p current_scopes(target)
      if call_scope && (value.present? || options[:allow_blank])

        current_scopes(target)[key] = value
        #        p current_scopes(target)
        #        p current_scopes(target)[key]
  ...
  def current_scopes(klazz)
    key = klazz.name
    @current_scopes ||= {}.with_indifferent_access
    @current_scopes[key] ||= {}.with_indifferent_access if @current_scopes[key].blank?
    @current_scopes[key]
  end
end
